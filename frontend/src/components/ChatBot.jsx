...
import { FaRegUser, FaTimes, FaPaperPlane, FaExpand, FaCompress, FaComments } from "react-icons/fa";
...
<stop offset="0.364" stop-color="#BA68C8"/>
      ...
      {isOpen && (
        <div
          className={`fixed bottom-5 right-5 bg-white shadow-lg border border-gray-300 flex flex-col transition-all duration-300
          ${isMaximized ? "w-1/4 h-3/4" : "w-80 h-96"} `}
        >
          {/* Chat Header */}
          <div className="bg-gradient-to-r from-purple-500 to-pink-500 p-4 flex justify-between items-center text-white">
            ...
            <div className="flex gap-3">
              {isMaximized ? (
                <FaCompress className="cursor-pointer text-lg" onClick={toggleMaximize} />
              ) : (
                <FaExpand className="cursor-pointer text-lg" onClick={toggleMaximize} />
              )}
              <FaTimes className="cursor-pointer text-lg" onClick={toggleChat} />
            </div>
          </div>
  ...
  />
                ...
                <div
                  className={`px-4 py-2 rounded-lg text-sm ${
                    msg.sender === "bot" ? "bg-gray-200 text-black" : "bg-purple-500 text-white"
                  }`}
                >
                  {msg.text}
                </div>
                {msg.sender === "user" && (
  <div className="w-8 h-8 flex items-center justify-center rounded-full border-2 border-purple-700 bg-white-900 text-purple-700 ml-2">
    <FaRegUser className="text-lg" />
          ...
          <div className="border-t p-2 flex items-center">
            ...
            />
            <button onClick={sendMessage} className="p-2 text-purple-600 hover:text-purple-700">
              <FaPaperPlane />
            </button>
          </div>
        </div>
