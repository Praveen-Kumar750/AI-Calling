...
const ClientNavbar = () => {
  ...
  // }, [loginDateTime]); // Ensure reactivity
  

  useEffect(() => {
    let storedLoginTime = sessionStorage.getItem("loginDateTime");

    if (!storedLoginTime) {
      const currentTime = new Date().toISOString();
      sessionStorage.setItem("loginDateTime", currentTime);
      storedLoginTime = currentTime;
    }
  ...
  useEffect(() => {
    ...
    updateActiveDuration(); // Update immediately when the component mounts
    const interval = setInterval(updateActiveDuration, 15000); // Update every 15 seconds
  ...
  return (
    <nav className="flex justify-between items-center p-4 bg-gray-800 relative w-full">
      ...
      {/* Desktop Nav */}
      <div
        className={`md:flex md:space-x-6 absolute md:relative top-16 md:top-0 left-0 w-full md:w-auto bg-gray-800 md:bg-transparent flex-col md:flex-row items-center transition-transform duration-300 ease-in-out ${
        ...
        {navItems.map(({ name, path }) => (
          <button
            ...
            className={`text-[20px] md:text-[30px] my-2 md:my-0 px-4 py-2 text-white w-full md:w-auto cursor-pointer text-center ${
              (name === "Incoming" &&
                location.pathname.startsWith("/incoming-")) ||
              (name === "Outgoing" &&
                location.pathname.startsWith("/outgoing-")) ||
              (name === "Billing" &&
        ...
        <div className="relative">
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg py-2 text-sm md:text-base">
              <p className="px-2 py-2 text-gray-800 font-bold">Login Time:</p>
              ...
              <p className="px-2 py-2 text-gray-800 font-bold">
                Active Time: <span className="font-medium text-green-800">{activeDuration}</span>
              </p>
              <button
