import BillingMessageLog from "../models/BillingMessagesModel.js";
...
export const getAllBillingLogs = async (req, res) => {
  try {
    const logs = await BillingMessageLog.find();
...
export const getBillingLogById = async (req, res) => {
  try {
    const log = await BillingMessageLog.findById(req.params.id);
...
export const getBillingLogsByNumber = async (req, res) => {
  try {
    const logs = await BillingMessageLog.find({ contactNo: req.params.number });
    res.status(200).json(logs);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const deleteBillingLog = async (req, res) => {
  try {
    const log = await BillingMessageLog.findByIdAndDelete(req.params.id);
    if (!log) return res.status(404).json({ message: "Log not found" });
